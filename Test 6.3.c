...
int atualizarMatriz() {
    ...
    while (playerInputPermission == 1)  // playerInput ? a vari?vel que controla o loop de entrada do jogador
    {
        ...
        if (menuOpen == 0) { // Verifica se o menu n?o est? aberto
            if (_kbhit()) {
                char comando = _getch();
                int newX = playerX;
                int newY = playerY;
            ...
            if (_kbhit()) {
                ...
                if (comando == 'w' || comando == 'W') {
                    ...
                    system ("cls");
                    mainMenu(); // Volta para o menu principal
